...
def make_3d_fig(df_display, color_col, simple_hover=True, cloud=None, inversions=None, llj_layers=None, show_cloud=False, cloud_opacity=0.35, show_inversion=False, show_llj=False, show_direction=True, vertical_mode="실제 고도", map_floor_trace=None, floor_grid_traces=None, anim_row=None, anim_trail_df=None, show_sonde_icon=True, scene_context_factor: float = 1.0, ns_context_factor: float = 1.0, show_isotherm_surfaces: bool = False, isotherm_opacity: float = 0.22, show_place_labels: bool = True):
    ...
    if floor_grid_traces:
        for tr in floor_grid_traces:
            fig.add_trace(tr)
    floor_z_label, _ = floor_z_for_display(df_display, vertical_mode)
    fig = _add_place_labels_to_fig(fig, df_display, floor_z_label, show=show_place_labels)
    fig.add_trace(go.Scatter3d(
...
def make_sonde_animation_fig(anim_df: pd.DataFrame, vertical_mode: str = "실제 고도", map_floor_trace=None, floor_grid_traces=None, scene_context_factor: float = 1.0, ns_context_factor: float = 1.0, show_isotherm_surfaces: bool = False, isotherm_opacity: float = 0.22, show_place_labels: bool = True):
    ...
    if floor_grid_traces:
        for tr in floor_grid_traces:
            fig.add_trace(tr)
    floor_z_label, _ = floor_z_for_display(anim_df, vertical_mode)
    fig = _add_place_labels_to_fig(fig, anim_df, floor_z_label, show=show_place_labels)
