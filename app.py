...
def make_sonde_animation_fig(anim_df: pd.DataFrame, vertical_mode: str = "실제 고도", map_floor_trace=None, floor_grid_traces=None):
    ...
    x_range = padded_range(anim_df["x_km"], pad_ratio=0.18, min_pad=0.8)
    y_range = padded_range(anim_df["y_km"], pad_ratio=0.18, min_pad=0.8)
...
map_info_text = ""
map_profiles = {
    "표준": dict(out_size=1100, pad_ratio=0.16, min_span_deg=0.16, max_tiles=6),
    "지역명 선명": dict(out_size=1500, pad_ratio=0.12, min_span_deg=0.10, max_tiles=8),
    "고해상도": dict(out_size=1800, pad_ratio=0.10, min_span_deg=0.07, max_tiles=10),
}
...
map_profile = map_profiles.get(map_detail_effective, map_profiles["지역명 선명"])
if show_map_floor:
    try:
        ...
        map_info_text = f"바닥면 지도: OpenStreetMap 정적 타일 기반, zoom {map_zoom}, 선명도 {map_detail_effective}, 표면 {map_grid_cells_effective}셀{suffix}, © OpenStreetMap contributors"
    except Exception as e:
...
with tab_anim:
    ...
    try:
        ...
        st.caption(f"애니메이션 프레임: {len(anim_df)}개 / 표시 간격: {anim_seconds}초 기준 / 지도는 현재 설정을 사용하되 Log-P에서는 자동 경량화가 적용됩니다.")
        anim_fig = make_sonde_animation_fig(anim_df, vertical_mode=vertical_mode, map_floor_trace=map_floor_trace, floor_grid_traces=floor_grid_traces)
